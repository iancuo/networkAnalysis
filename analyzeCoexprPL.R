...
results_highCPMgenes=read.csv("resultsCoexpr_PL/resultsDEDV_highCPM.csv")
...
fileConnSummary<-file("resultsCoexpr_PL/SummaryResultsCoexpr.txt",  open="at")
...
save(adjCoexpr, colorsCoexpr,modulesCoexpr, adj_HSCC_H,adj_HSCC_L, file="data/adjCoexprModules_PL.RData")
...
write.csv(cellTypeEnrichment, file="resultsCoexpr_PL/cellTypeEnrich.csv", append=T)
...
try(dir.create("resultsCoexpr_PL/moduleGeneList"), silent = T)
...
for (module in modulesCoexpr){
  ...
  write.csv(currModuleInfo, file=paste("resultsCoexpr_PL/moduleGeneList/module_", module, ".csv", sep=""), row.names=F, col.names=F)  
}
#############################################################################
# GO annotations

load("/home/dan/workDir/networkAnalysis/data/transcriptInfoMouse.RData")
annotateMouseModulesGO(colorsCoexpr, transcriptInfoMouse, type="Coexpr_PL")
##############################################################################
...
diffEdgesPL = diffEdges(rawAdj1, rawAdj2, n1=dim(HSCC_H)[2], n2=dim(HSCC_L)[2], pThreshold=0.01, adjThreshold=0.5, nCores=7)
  
save(diffEdgesPL, file="resultsCoexpr_PL/diffEdgesPL.RData")
load("resultsCoexpr_PL/diffEdgesPL.RData")

totalEdges=(length(geneNames))^2
affectedEdges=sum(diffEdgesPL)
edgeChangeRate=affectedEdges/totalEdges

geneChangeEdgeCount=rowSums(diffEdgesPL)
...
summaryResultsPL=cbind(colorsCoexpr, results_DEDVNet[,c(1:3, 5:12)], round(coexprConnConsensus[,"kWithin"],3), round(coexprConnLow[,"kWithin"],3), round(coexprConnHigh[,"kWithin"],3),round(pValuesEdgeChange,3), round(sortedAdjustedPvals_DW[geneNames],3),geneChangeEdgeCount )
colnames(summaryResultsPL)
colnames(summaryResultsPL)=c("module","geneName", "logFC", "logCPM", "mean.counts.L","mean.counts.H","p.val.DE", "adj.p.DE" , "sd.counts.L","sd.counts.H","p.val.DV", "adj.p.DV","modular conn Consensus","modular conn Low", "modular conn High","p.val.DW", "adj.p.DW", "changed edge count")


#sanity checks
plot(summaryResultsPL[,"logFC"], -log10(summaryResultsPL[,"p.val.DE"]))
plot(summaryResultsPL[,"changed edge count"], -log10(summaryResultsPL[,"p.val.DW"]))
plot(summaryResultsPL[,"changed edge count"], -log10(summaryResultsPL[,"adj.p.DW"]))

write.csv(summaryResultsPL, file="resultsCoexpr_PL/tableResultsPL.csv")


#####################################################################################
deGenes=geneNames[summaryResultsPL[,"p.val.DE"] < 0.01]
dvGenes=geneNames[summaryResultsPL[,"p.val.DV"] < 0.01]
dwGenes=geneNames[summaryResultsPL[,"p.val.DW"] < 0.01]
