...
results_highCPMgenes=read.csv("resultsCoexpr_SH/resultsDEDV_highCPM.csv")
...
save(adjCoexpr, colorsCoexpr,modulesCoexpr, adj_HSCC_H,adj_HSCC_L, file="data/adjCoexprModules_SH.RData")
...
write.csv(cellTypeEnrichment, file="resultsCoexpr_SH/cellTypeEnrich.csv", append=T)
...
try(dir.create("resultsCoexpr_SH/moduleGeneList"), silent = T)
...
for (module in modulesCoexpr){
  ...
  write.csv(currModuleInfo, file=paste("resultsCoexpr_SH/moduleGeneList/module_", module, ".csv", sep=""), row.names=F, col.names=F)  
}
#############################################################################
# GO annotations

load("/home/dan/workDir/networkAnalysis/data/transcriptInfoMouse.RData")
annotateMouseModulesGO(colorsCoexpr, transcriptInfoMouse, type="Coexpr_SH")
##############################################################################
...
diffEdgesSH = diffEdges(rawAdj1, rawAdj2, n1=dim(HSCC_H)[2], n2=dim(HSCC_L)[2], pThreshold=0.01, adjThreshold=0.5, nCores=7)
  
save(diffEdgesSH, file="resultsCoexpr_SH/diffEdgesSH.RData")
load("resultsCoexpr_SH/diffEdgesSH.RData")

totalEdges=(length(geneNames))^2
affectedEdges=sum(diffEdgesSH)
edgeChangeRate=affectedEdges/totalEdges

geneChangeEdgeCount=rowSums(diffEdgesSH)
...
summaryResultsSH=cbind(colorsCoexpr, results_DEDVNet[,c(1:3, 5:12)], round(coexprConnConsensus[,"kWithin"],3), round(coexprConnLow[,"kWithin"],3), round(coexprConnHigh[,"kWithin"],3),round(pValuesEdgeChange,3), round(sortedAdjustedPvals_DW[geneNames],3),geneChangeEdgeCount )
colnames(summaryResultsSH)
colnames(summaryResultsSH)=c("module","geneName", "logFC", "logCPM", "mean.counts.L","mean.counts.H","p.val.DE", "adj.p.DE" , "sd.counts.L","sd.counts.H","p.val.DV", "adj.p.DV","modular conn Consensus","modular conn Low", "modular conn High","p.val.DW", "adj.p.DW", "changed edge count")


#sanity checks
plot(summaryResultsSH[,"logFC"], -log10(summaryResultsSH[,"p.val.DE"]))
plot(summaryResultsSH[,"changed edge count"], -log10(summaryResultsSH[,"p.val.DW"]))
plot(summaryResultsSH[,"changed edge count"], -log10(summaryResultsSH[,"adj.p.DW"]))

write.csv(summaryResultsSH, file="resultsCoexpr_SH/tableResultsSH.csv")


#####################################################################################
deGenes=geneNames[summaryResultsSH[,"p.val.DE"] < 0.01]
dvGenes=geneNames[summaryResultsSH[,"p.val.DV"] < 0.01]
dwGenes=geneNames[summaryResultsSH[,"p.val.DW"] < 0.01]
