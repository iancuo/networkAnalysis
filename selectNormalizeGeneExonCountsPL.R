...
try(dir.create("resultsCoexpr_PL"), silent = F)
try( dir.create("figuresCoexpr_PL"), silent = F)
try(dir.create("resultsCoSplicEx_PL"), silent = F)
try( dir.create("figuresCoSplicEx_PL"), silent = F)

# read raw data - can be improved by using read.table from original .txt file
geneReadsRaw=as.matrix(read.table("RNASeq019 HSCC Alex/PL/RNASeq019_PL_mm10_gene_reads_not_normalized.txt"))
...
sampleKey=read.csv("RNASeq019 HSCC Alex/PL/PLsampleKey.csv", header=T)
sampleKey[,"Prelimbic"]=paste("S", sampleKey[,"Prelimbic"], sep = "")
...
samplesHigh=sampleKey[grep("H",sampleKey[,"Line"] ), "Prelimbic"]
samplesLow=sampleKey[grep("L",sampleKey[,"Line"] ), "Prelimbic"]
...
fileConnSummary<-file("resultsCoexpr_PL/SummaryResultsCoexpr.txt",  open="wt")
...
write.csv(geneNamesDE, file="resultsCoexpr_PL/geneNamesDE.csv")
write.csv(geneNamesDV, file="resultsCoexpr_PL/geneNamesDV.csv")
...
write.csv(results_highCPMgenes, file="resultsCoexpr_PL/resultsDEDV_highCPM.csv")
...
###################################################################################
exonCounts=read.table("RNASeq019 HSCC Alex/PL/RNASeq019_PL_mm10_exon_reads_not_normalized.txt")
...
save(canberraListExons, file="resultsCoSplicEx_PL/canberraListExonsPL.RData")
# load("resultsCoSplicEx_PL/canberraListExonsPL.RData")
########################################################################################################
...
save(selectedGeneCounts, canberraListSelected,adjCoSplicEx,selectedExonCounts, exonGeneNameSelected, groupSelection, samplesHigh, samplesLow, file="selectedData_PL.RData")
